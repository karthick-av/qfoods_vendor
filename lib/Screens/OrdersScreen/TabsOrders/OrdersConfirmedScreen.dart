...
class OrdersConfirmedScreen extends StatefulWidget {
  const OrdersConfirmedScreen({super.key});

  @override
  State<OrdersConfirmedScreen> createState() => _OrdersConfirmedScreenState();
}

class _OrdersConfirmedScreenState extends State<OrdersConfirmedScreen> {
...
void initState(){
  ...
  WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
   ...
   if((detailProvider?.confirmed?.length ?? 0) == 0 && detailProvider.lconfirmed == false){
...
try{
       String url = "${ApiServices.orders}${restaurant_id}?status=2&page=${current_page}&per_page=${per_page}&${filter}";
   print(url);
    var response = await http.get(Uri.parse(url));
orderProvider.updateConfirmed();
          setState(() {
    ApiCallDone = false;
  });
...
setState(() {
    ...
    current_page = current_page + 1;
      ...
      if(type == "init"){
      orderProvider.addAllConfirmed(_list);   
      }else{
       orderProvider.addConfirmed(_list);   
...
void filterHandler(String selectedDate, String fromDate, String toDate){
 final detailProvider = Provider.of<OrdersProvider>(context, listen: false);
  ...
  ResetState();
  detailProvider.addAllConfirmed([]);
...
getDetailHandler("init", val);
  ...
  final list = Provider.of<OrdersProvider>(context, listen: true).confirmed;
