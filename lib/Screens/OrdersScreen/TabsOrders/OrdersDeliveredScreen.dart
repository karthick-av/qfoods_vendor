...
class OrdersDeliveredScreen extends StatefulWidget {
  const OrdersDeliveredScreen({super.key});

  @override
  State<OrdersDeliveredScreen> createState() => _OrdersDeliveredScreenState();
}

class _OrdersDeliveredScreenState extends State<OrdersDeliveredScreen> {
...
void initState(){
  ...
  WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
   ...
   if((detailProvider?.delivered?.length ?? 0) == 0 && detailProvider?.ldelivered == false){
...
try{
       String url = "${ApiServices.orders}${restaurant_id}?isDelivered=true&page=${current_page}&per_page=${per_page}&${filter}";
   print(url);
    var response = await http.get(Uri.parse(url));
orderProvider.updateDelivered();
          setState(() {
    ApiCallDone = false;
  });
...
setState(() {
    ...
    current_page = current_page + 1;
      ...
      if(type == "init"){
      orderProvider.addAllDelivered(_list);   
      }else{
       orderProvider.addDelivered(_list);   
...
void filterHandler(String selectedDate, String fromDate, String toDate){
 final detailProvider = Provider.of<OrdersProvider>(context, listen: false);
  ...
  ResetState();
  detailProvider.addAllDelivered([]);
...
getDetailHandler("init", val);
  ...
  final list = Provider.of<OrdersProvider>(context, listen: true).delivered;
