...
class OrdersCancelledScreen extends StatefulWidget {
  const OrdersCancelledScreen({super.key});

  @override
  State<OrdersCancelledScreen> createState() => _OrdersCancelledScreenState();
}

class _OrdersCancelledScreenState extends State<OrdersCancelledScreen> {
...
void initState(){
  ...
  WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
   ...
   if((detailProvider?.cancelled?.length ?? 0) == 0 && detailProvider?.lcancelled ==false){
...
try{
       String url = "${ApiServices.orders}${restaurant_id}?isCancelled=true&page=${current_page}&per_page=${per_page}&${filter}";
   print(url);
    var response = await http.get(Uri.parse(url));
orderProvider.updatecancelled();
          setState(() {
    ApiCallDone = false;
  });
...
setState(() {
    ...
    current_page = current_page + 1;
      ...
      if(type == "init"){
      orderProvider.addAllCancelled(_list);   
      }else{
       orderProvider.addCanelled(_list);   
...
void filterHandler(String selectedDate, String fromDate, String toDate){
 final detailProvider = Provider.of<OrdersProvider>(context, listen: false);
  ...
  ResetState();
  detailProvider.addAllCancelled([]);
...
getDetailHandler("init", val);
  ...
  final list = Provider.of<OrdersProvider>(context, listen: true).cancelled;
