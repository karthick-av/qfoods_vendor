...
void initState(){
  ...
  WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
   ...
   if((detailProvider?.orders?.length ?? 0) == 0){
...
try{
       String url = "${ApiServices.orders}${restaurant_id}?page=${current_page}&per_page=${per_page}&${filter}";
   print(url);
...
setState(() {
    ...
    current_page = current_page + 1;
      ...
      if(type == "init"){
      orderProvider.addAll(_list);   
      }else{
       orderProvider.add(_list);   
...
getDetailHandler("init", val);
  ...
  final list = Provider.of<OrdersProvider>(context, listen: true).orders;
    ...
    color: Colors.white, // <-- SEE HERE
        ...
        elevation: 0.5,
        title: Text("Orders", style: TextStyle(fontFamily: FONT_FAMILY, fontSize: ScreenUtil().setSp(16.0), fontWeight: FontWeight.bold,color: AppColors.whitecolor),),
      ),
      body: SafeArea(
